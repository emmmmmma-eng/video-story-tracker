...
type View = "overview" | "records" | "accounts" | "patterns" | "remix";
...
export default function Home() {
  ...
  const [selectedRemixId, setSelectedRemixId] = useState("");
  ...
  function generateRemixCopy(source: Entry, idea: string): string[] {
    const subject = idea.trim() || "把這個主題改寫成我的版本";
    const structure = `${source.hookType} → ${source.endingType}`;
    const title = source.title || "這支影片的核心故事";
    const core = source.contrastStructure || source.notes || "把原本的情緒轉成自己的生活經驗";
    return [
      `【版本一｜身份反差】\n${subject}。\n\n你以為「${title}」只是在講一件事，其實真正讓人停下來的，是「${core}」。\n\n我也曾經以為自己只要把事情做好就夠了，後來才發現，最難面對的往往不是結果，而是那個一直被我忽略的自己。\n\n如果你也有過類似的時刻，先別急著滑走。`,
      `【版本二｜懸念推進】\n${subject}。\n\n我原本以為這只是一個普通的故事，直到我發現：${core}。\n\n前面看起來像是在分享經驗，後面才知道，真正的答案藏在最後一個細節裡。也許你現在正在經歷的事，正好能在這裡找到一點提醒。\n\n你會怎麼選？`,
      `【版本三｜生活口語】\n${subject}。\n\n最近我一直在想，為什麼有些內容明明很簡單，卻會讓人一直記得？答案可能就在「${core}」。\n\n不是因為它說了多厲害的道理，而是它讓我們想起自己也有過那個瞬間。這一次，我想用自己的方式，把這段感受說清楚。\n\n你最近也有這種感覺嗎？`,
      `【版本四｜金句收束】\n${subject}。\n\n${title}真正留下來的，不只是故事本身，而是它提醒我們：很多時候，我們以為自己在等待一個更好的答案，其實是在等自己終於願意承認心裡的答案。\n\n這次我會沿用「${structure}」的節奏，但換成我的經驗、我的語氣，讓它變成一個真正屬於我的故事。\n\n有些話，現在說出口，剛剛好。`,
    ];
  }

  function createRemix() {
    const source = entries.find((entry) => entry.id === selectedRemixId);
    if (!source) {
      notify("請先選擇一篇喜歡的原文案");
      return;
    }
    ...
    window.setTimeout(() => {
      setRemixResults(generateRemixCopy(source, remixIdea));
  ...
  const navItems: { key: View; label: string; icon: LucideIcon; hint: string }[] = [
    ...
    { key: "remix", label: "二創文案", icon: Sparkles, hint: "依照喜歡的結構生成" },
  ];
